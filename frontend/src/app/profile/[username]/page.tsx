...
const ProfilePage = ({ params }: { params: { username: string } }) => {
  ...
  const handleDeleteAccount = async () => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${user?.username}`);
      router.push("/sign-in");
    } catch (error) {
  ...
  const handleUnfollow = async () => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/users/unfollow`, {
        data: {
          id: currentUser?.id,
          followingId: currentProfile?.id,
        },
  ...
  const editLogs = async (id: string, description: string, level: string) => {
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/deepWorkLogs/${id}`, {
  ...
  useEffect(() => {
    const fetchFollower = async () => {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/users/followers/` + currentProfile?.id
      );
    ...
    const fetchFollowing = async () => {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/users/following/` + currentProfile?.id
      );
    ...
    const fetchDeepWorkLogs = async () => {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/users/${username}/deepWorkLogs`
      );
    ...
    const fetchData = async () => {
      if (user?.username !== username) {
        const thirdResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/users/followed/` +
            user?.username +
            `/` +
            username
        );
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/deepWorkLogs/${id}`);
